...
const Card: React.FC<CardProps> = ({ user, onSwipe, currentImageIndex, isTopCard, onImageChange, cardStyle }) => {
        ...
        if (state === State.END) {
            ...
            if (shouldSwipeRight || shouldSwipeLeft) {
                ...
                translateX.value = withTiming(width * (direction === 'right' ? 1.5 : -1.5), { duration: 300 }, () => {
                    runOnJS(onSwipe)(user._id, direction);
                });
            } else {
    ...
    const CardView = ({ user, currentImageIndex }: { user: any, currentImageIndex: number }) => (
        <View style={styles.card}>
            <Animated.Image
                source={{ uri: user.profilePictures[currentImageIndex] || 'https://via.placeholder.com/500' }}
            ...
            <View style={styles.imageIndicatorContainer}>
                {user.profilePictures.map((_: any, index: number) => (
            ...
            <Animated.View style={[styles.choiceOverlay, { borderColor: '#4CCC93' }, { opacity: interpolate(translateX.value, [0, SWIPE_THRESHOLD], [0, 1], Extrapolate.CLAMP) }]}>
                <Text style={[styles.choiceText, { color: '#4CCC93' }]}>LIKE</Text>
            </Animated.View>
            <Animated.View style={[styles.choiceOverlay, { borderColor: '#FF6B6B', right: null, left: 20 }, { opacity: interpolate(translateX.value, [-SWIPE_THRESHOLD, 0], [1, 0], Extrapolate.CLAMP) }]}>
                <Text style={[styles.choiceText, { color: '#FF6B6B' }]}>NOPE</Text>
            </Animated.View>
        </View>
    );
    
    // Prikazujemo kartice koje su ispod glavne sa manjim zIndexom i skalom
    if (!isTopCard) {
        return (
    ...
    // Glavna (top) kartica dobija gesture handlere i veći zIndex
    return (
        <PanGestureHandler
            ref={panRef}
