// app/profile/edit/loveStyle.tsx
...
export default function LoveStyleScreen() {
  ...
  useEffect(() => {
    if (params.currentLoveStyle) {
      setSelectedOption(params.currentLoveStyle as string);
    }
  }, [params.currentLoveStyle]);

  const handleSave = useCallback(() => {
    if (selectedOption) {
      router.navigate({
        pathname: '/profile/edit-profile', // Ispravna putanja nazad
        params: { updatedLoveStyle: selectedOption },
      });
    } else {
      Alert.alert("Greška", "Molimo odaberite ljubavni stil.");
    }
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Ljubavni stil</Text>
        <TouchableOpacity onPress={handleSave} style={styles.saveButton}>
          <Text style={styles.saveButtonText}>Sačuvaj</Text>
        </TouchableOpacity>
      </View>
      <View style={styles.container}>
        {loveStyleOptions.map((option) => (
          <TouchableOpacity
            key={option}
