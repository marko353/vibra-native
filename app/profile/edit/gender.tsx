...
export default function GenderScreen() {
  ...
  useEffect(() => {
    if (params.currentGender) {
      setSelectedOption(params.currentGender as string);
    }
  }, [params.currentGender]);

  const handleSave = useCallback(() => {
    if (selectedOption) {
      router.navigate({
        pathname: '/profile/edit-profile', // Ispravna putanja nazad
        params: { updatedGender: selectedOption },
      });
    } else {
      Alert.alert("Greška", "Molimo odaberite pol.");
    }
