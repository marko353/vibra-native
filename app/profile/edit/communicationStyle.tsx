// app/profile/edit/communicationStyle.tsx
...
const communicationStyleOptions = [
  "Volim duge razgovore",
  "Više volim poruke",
  "Uživo je najbolje",
  "Prilagođavam se",
  "Nisam previše komunikativan/na",
];
...
export default function CommunicationStyleScreen() {
  ...
  useEffect(() => {
    if (params.currentCommunicationStyle) {
      setSelectedOption(params.currentCommunicationStyle as string);
    }
  }, [params.currentCommunicationStyle]);

  const handleSave = useCallback(() => {
    if (selectedOption) {
      router.navigate({
        pathname: '/profile/edit-profile', // Ispravna putanja nazad
        params: { updatedCommunicationStyle: selectedOption },
      });
    } else {
      Alert.alert("Greška", "Molimo odaberite stil komunikacije.");
    }
