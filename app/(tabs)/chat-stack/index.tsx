...
export default function ChatScreen() {
      ...
      queryFn: async () => {
        ...
        try {
          const response = await axios.get(
            `${API_BASE_URL}/api/user/my-matches`,
            {
          ...
          console.log(
            "📦 [ChatScreen] SIROVI PODACI SA SERVERA:",
  ...
  useEffect(() => {
    ...
    const handleRefresh = (payload: any) => {
      console.log(
        "⚡ [ChatScreen] Socket događaj primljen, osvežavam listu za 800ms:",
        payload
      );
  ...
  const handleOpenChat = (
    chatUser: Match | Conversation["user"],
    chatId: string
  ) => {
    console.log("🚀 [ChatScreen] Otvaram chat:", {
      chatId,
      user: chatUser.fullName,
    });

    const match = newMatches.find((m) => m.chatId === chatId);
    const conv = conversations.find((c) => c.chatId === chatId);
    if (match?.has_unread || conv?.has_unread)
  ...
  return (
    <View style={styles.container}>
      <Header onFilterClick={undefined} />
      {isLoading && !data ? (
          ...
          {newMatches.length === 0 && conversations.length === 0 ? (
            <View style={styles.center}>
              ...
              <Text style={styles.emptyText}>
                Kada se spojiš sa nekim ili dobiješ poruku, videćeš ih ovde.
              </Text>
            </View>
          ) : (
            <>
                  ...
                  >
                    {newMatches.map((match) => (
                      ...
                      >
                        ...
                        {match.has_unread && (
                          <View style={styles.notificationDotMatch} />
                        )}
              ...
              <View style={styles.sectionMessages}>
                ...
                {conversations.length === 0 && newMatches.length > 0 && (
                  <Text style={styles.noMessagesText}>
                    Još uvek nemaš aktivnih razgovora.
                  </Text>
                )}
                  ...
                  <React.Fragment key={conv.chatId}>
                    ...
                    >
                      <View style={styles.avatarContainer}>
                        ...
                        />
                        {conv.has_unread && (
                          <View style={styles.notificationDotConv} />
                        )}
                      </View>
                      <View style={styles.conversationTextContainer}>
                        <Text
                          style={[
                            ...
                            conv.has_unread && styles.unreadText,
                          ]}
                          numberOfLines={1}
                        >
                          {conv.user.fullName}
                      ...
                      <Text
                        style={[
                          styles.timestamp,
                          conv.has_unread && styles.unreadTimestamp,
                        ]}
                      >
