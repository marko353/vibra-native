...
export default function HomeScreen() {
  const { isVisible, showModal, hideModal, filterValues, setFilterValues } =
    useFilterModal();
    ...
    async (targetUserId: string, direction: "left" | "right") => {
      ...
      try {
        ...
        if (response.data.match) {
          ...
          queryClient.invalidateQueries({
            queryKey: ["matches-and-conversations", user?.id],
          });
    ...
    async (message: string) => {
      ...
      if (trimmedMessage) {
        ...
        queryClient.setQueryData(
          ["matches-and-conversations", user.id],
          (oldData: any) => {
            if (!oldData) return oldData;

            const newMatches = (oldData.newMatches || []).filter(
              (m: any) => m._id !== targetUserId
            );
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      ...
      {/* Dodaj LikeFilterModal na kraj JSX-a */}
      <LikeFilterModal
        visible={isVisible}
        onClose={hideModal}
