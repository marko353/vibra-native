...
const OPTIONS = [
  { value: 'Dog', emoji: '🐶', description: 'I have or love dogs.' },
  { value: 'Cat', emoji: '🐱', description: 'I have or love cats.' },
  { value: 'Fish', emoji: '🐠', description: 'I keep fish as pets.' },
  { value: 'Bird', emoji: '🐦', description: 'I have or love birds.' },
  { value: 'Reptile', emoji: '🦎', description: 'I have or love reptiles.' },
  { value: 'None', emoji: '🚫', description: 'I don\'t have any pets.' },
  { value: 'Doesn\'t matter', emoji: '🤷', description: 'I\'m open to any pet situation.' },
];
...
export default function PetsScreen() {
  ...
  useEffect(() => {
    if (userProfile?.pets) setSelected(userProfile.pets);
  }, [userProfile]);

  const hasChanges = useMemo(() =>
    selected !== userProfile?.pets,
  ...
  const mutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      setProfileField('pets', variables.value);
      queryClient.setQueryData(['userProfile', user?.id], (old: any) =>
        old ? { ...old, pets: variables.value } : old
      );
      router.back();
    },
    onError: (error: any) => {
  ...
  const handleSave = () => {
    ...
    mutation.mutate({ field: 'pets', value: selected });
  };
  ...
  return (
    <View style={[modalStyles.container, { paddingBottom: insets.bottom || 16 }]}>
      ...
      {isProfileLoading ? (
        ...
        <>
          <Text style={styles.subtitle}>Do you have any pets?</Text>
          <FlatList
            data={OPTIONS}
