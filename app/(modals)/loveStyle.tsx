...
export default function LoveStyleScreen() {
    ...
    useEffect(() => {
        if (userProfile?.loveStyle) {
            setSelectedStyle(userProfile.loveStyle);
        }
    }, [userProfile]);

    const hasChanges = useMemo(() => selectedStyle !== userProfile?.loveStyle, [selectedStyle, userProfile]);
    ...
    const updateProfileMutation = useMutation({
        ...
        onSuccess: (data, variables) => {
            const fieldName = 'loveStyle' as const;
        ...
        onError: (error: any) => {
            console.error('Greška pri čuvanju jezika ljubavi:', error.response?.data || error.message);
            Alert.alert('Greška', `Došlo je do greške prilikom čuvanja jezika ljubavi: ${error.response?.data?.message || error.message}`);
        },
    });

    const handleSave = () => {
        if (!selectedStyle) {
            Alert.alert('Greška', 'Molimo odaberite opciju za jezik ljubavi.');
            return;
        }
        if (!hasChanges || updateProfileMutation.isPending || isProfileLoading) return;
        updateProfileMutation.mutate({ field: 'loveStyle', value: selectedStyle });
    };

    const renderOption = (option: string) => {
        const isSelected = option === selectedStyle;
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle}>Jezik ljubavi</Text>
                <TouchableOpacity
                    style={styles.saveBtn}
                    onPress={handleSave}
                    disabled={!hasChanges || updateProfileMutation.isPending || isProfileLoading || !selectedStyle}
                >
                    {updateProfileMutation.isPending ? (
                        ...
                        <Text style={[
                            styles.saveBtnText,
                            { color: (!hasChanges || !selectedStyle) ? COLORS.textSecondary : COLORS.primary }
                        ]}>
                            Sačuvaj
                        </Text>
                    )}
            ...
            {isProfileLoading ? (
                ...
                <View style={styles.content}>
                    <Text style={styles.label}>Kako najviše voliš da pokazuješ i primaš ljubav?</Text>
