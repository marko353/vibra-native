...
import { useProfileContext } from '../../context/ProfileContext'; // <-- DODAT IMPORT
...
interface UserProfile { sexualOrientation: string | null; [key: string]: any; }
...
export default function SexualOrientationScreen() {
    ...
    useEffect(() => {
        if (userProfile?.sexualOrientation) {
            setSelectedOrientation(userProfile.sexualOrientation);
        }
    }, [userProfile]);

    const hasChanges = useMemo(() => selectedOrientation !== userProfile?.sexualOrientation, [selectedOrientation, userProfile]);
    ...
    const updateProfileMutation = useMutation({
        ...
        onSuccess: (data, variables) => {
            const fieldName = 'sexualOrientation' as const;
            ...
            setSelectedOrientation(newValue);

            // 4. Zatvaranje modala
            router.back();
        },
        onError: (error: any) => {
            console.error('Greška pri čuvanju seksualne orijentacije:', error.response?.data || error.message);
            Alert.alert('Greška', `Došlo je do greške prilikom čuvanja seksualne orijentacije: ${error.response?.data?.message || error.message}`);
        },
    });

    const handleSave = () => {
        if (!selectedOrientation) {
            Alert.alert('Greška', 'Molimo odaberite opciju za seksualnu orijentaciju.');
            return;
        }
        if (!hasChanges || updateProfileMutation.isPending || isProfileLoading) return;
        updateProfileMutation.mutate({ field: 'sexualOrientation', value: selectedOrientation });
    };

    const renderOption = (option: string) => {
        const isSelected = option === selectedOrientation;
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle}>Seksualna orijentacija</Text>
                <TouchableOpacity
                    style={styles.saveBtn}
                    onPress={handleSave}
                    disabled={!hasChanges || updateProfileMutation.isPending || isProfileLoading || !selectedOrientation}
                >
                    {updateProfileMutation.isPending ? (
                        ...
                        <Text style={[
                            styles.saveBtnText,
                            { color: (!hasChanges || !selectedOrientation) ? COLORS.textSecondary : COLORS.primary }
                        ]}>
                            Sačuvaj
                        </Text>
                    )}
            ...
            {isProfileLoading ? (
                ...
                <View style={styles.content}>
                    <Text style={styles.label}>Koja je tvoja seksualna orijentacija?</Text>
