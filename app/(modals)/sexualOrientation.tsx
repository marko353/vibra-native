...
const OPTIONS = [
  { value: 'Straight', emoji: '🧡', description: 'I identify as heterosexual.' },
  { value: 'Gay', emoji: '🏳️‍🌈', description: 'I identify as gay.' },
  { value: 'Bisexual', emoji: '💜', description: 'I identify as bisexual.' },
  { value: 'Prefer not to say', emoji: '🤷', description: 'I prefer to keep this private.' },
];
...
export default function SexualOrientationScreen() {
  ...
  useEffect(() => {
    if (userProfile?.sexualOrientation) setSelected(userProfile.sexualOrientation);
  }, [userProfile]);

  const hasChanges = useMemo(() =>
    selected !== userProfile?.sexualOrientation,
  ...
  const mutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      setProfileField('sexualOrientation', variables.value);
      queryClient.setQueryData(['userProfile', user?.id], (old: any) =>
        old ? { ...old, sexualOrientation: variables.value } : old
      );
      router.back();
    },
    onError: (error: any) => {
  ...
  const handleSave = () => {
    ...
    mutation.mutate({ field: 'sexualOrientation', value: selected });
  };
  ...
  return (
    <View style={[modalStyles.container, { paddingBottom: insets.bottom || 16 }]}>
      ...
      {isProfileLoading ? (
        ...
        <>
          <Text style={styles.subtitle}>What is your sexual orientation?</Text>
          <FlatList
            data={OPTIONS}
