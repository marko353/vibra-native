// app/(modals)/education.tsx - Implementacija kao standardni ekran (bez react-native-modal)
...
export default function EducationScreen() {
  ...
  // Pribavljanje najnovijih podataka o korisniku
  ...
  // Koristimo useFocusEffect da uvek dobijemo najnovije podatke kada se ekran prikaže
  useFocusEffect(
    useCallback(() => {
      if (user?.token) {
        refetch();
  ...
  // Sinhronizacija stanja sa podacima iz upita
  useFocusEffect(
    useCallback(() => {
      if (userProfile?.education) {
        setSelectedEducation(userProfile.education);
      }
    }, [userProfile])
  );

  // Proverava da li je došlo do promene u odnosu na početnu opciju
  const hasChanges = useMemo(() => selectedEducation !== userProfile?.education, [selectedEducation, userProfile]);
