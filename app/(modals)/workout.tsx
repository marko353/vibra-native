...
const OPTIONS = [
  { value: 'Never', emoji: '🛋️', description: 'Working out isn\'t really my thing.' },
  { value: 'Sometimes', emoji: '🚶', description: 'I exercise every now and then.' },
  { value: 'Often', emoji: '🏃', description: 'I work out on a regular basis.' },
  { value: 'Every day', emoji: '💪', description: 'I\'m a daily gym enthusiast!' },
];
...
export default function WorkoutScreen() {
  ...
  useEffect(() => {
    if (userProfile?.workout) setSelected(userProfile.workout);
  }, [userProfile]);

  const hasChanges = useMemo(() =>
    selected !== userProfile?.workout,
  ...
  const mutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      setProfileField('workout', variables.value);
      queryClient.setQueryData(['userProfile', user?.id], (old: any) =>
        old ? { ...old, workout: variables.value } : old
      );
      router.back();
    },
    onError: (error: any) => {
  ...
  const handleSave = () => {
    ...
    mutation.mutate({ field: 'workout', value: selected });
  };
  ...
  return (
    <View style={[modalStyles.container, { paddingBottom: insets.bottom || 16 }]}>
      ...
      {isProfileLoading ? (
        ...
        <>
          <Text style={styles.subtitle}>How often do you work out?</Text>
          <FlatList
            data={OPTIONS}
