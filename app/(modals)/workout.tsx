...
const options = [
    ...
    'Ponekad',
    'Često',
    'Spreman/a sam za teretanu svakodnevno!',
];

interface MutationPayload { field: string; value: any; }
interface UserProfile { workout: string | null; [key: string]: any; }
...
export default function WorkoutScreen() {
    ...
    useEffect(() => {
        if (userProfile?.workout) {
            setSelectedWorkout(userProfile.workout);
        }
    }, [userProfile]);

    const hasChanges = useMemo(() => selectedWorkout !== userProfile?.workout, [selectedWorkout, userProfile]);
    ...
    const updateProfileMutation = useMutation({
        ...
        onSuccess: (data, variables) => {
            const fieldName = 'workout' as const;
        ...
        onError: (error: any) => {
            console.error('Greška pri čuvanju statusa vežbanja:', error.response?.data || error.message);
            Alert.alert('Greška', `Došlo je do greške prilikom čuvanja statusa vežbanja: ${error.response?.data?.message || error.message}`);
        },
    });

    const handleSave = () => {
        if (!selectedWorkout) {
            Alert.alert('Greška', 'Molimo odaberite opciju za vežbanje.');
            return;
        }
        if (!hasChanges || updateProfileMutation.isPending || isProfileLoading) return;
        updateProfileMutation.mutate({ field: 'workout', value: selectedWorkout });
    };

    const renderOption = (option: string) => {
        const isSelected = option === selectedWorkout;
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle}>Vežbanje</Text>
                <TouchableOpacity
                    style={styles.saveBtn}
                    onPress={handleSave}
                    disabled={!hasChanges || updateProfileMutation.isPending || isProfileLoading || !selectedWorkout}
                >
                    {updateProfileMutation.isPending ? (
                        ...
                        <Text style={[
                            styles.saveBtnText,
                            { color: (!hasChanges || !selectedWorkout) ? COLORS.textSecondary : COLORS.primary }
                        ]}>
                            Sačuvaj
                        </Text>
                    )}
            ...
            {isProfileLoading ? (
                ...
                <View style={styles.content}>
                    <Text style={styles.label}>Koliko često vežbaš?</Text>
