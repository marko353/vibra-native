...
const COLORS = {
    ...
    inputBorder: '#D0D0D0',
    inputFocusedBorder: '#E91E63',
    saveButtonShadow: 'rgba(233, 30, 99, 0.4)',
};
...
export default function JobScreen() {
    ...
    const hasChanges = useMemo(() => jobTitle.trim() !== (userProfile?.jobTitle || '').trim(), [jobTitle, userProfile]);
    ...
    const updateProfileMutation = useMutation({
        ...
        onSuccess: (data, variables) => {
            const fieldName = 'jobTitle' as const;
            ...
            // 3. TRENUTNO AŽURIRANJE LOKALNOG STANJA MODALA (opciono, ali dobro za konzistentnost)
        ...
        onError: (error: any) => {
            console.error('Greška pri čuvanju posla:', error.response?.data || error.message);
            Alert.alert('Greška', `Došlo je do greške prilikom čuvanja posla: ${error.response?.data?.message || error.message}`);
        },
    });
    ...
    const handleSave = () => {
        ...
        if (!trimmedJobTitle) {
            Alert.alert('Greška', 'Molimo unesite zanimanje.');
            return;
        }
        if (!hasChanges || updateProfileMutation.isPending || isProfileLoading) return;
        updateProfileMutation.mutate({ field: 'jobTitle', value: trimmedJobTitle });
    };
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle}>Tvoje zanimanje</Text>
                <TouchableOpacity
                    style={styles.saveBtn}
                    onPress={handleSave}
                    disabled={!hasChanges || updateProfileMutation.isPending || isProfileLoading || !jobTitle.trim()}
                >
                    {updateProfileMutation.isPending ? (
                        ...
                        <Text style={[
                            styles.saveBtnText,
                            { color: (!hasChanges || !jobTitle.trim()) ? COLORS.textSecondary : COLORS.primary }
                        ]}>
                            Sačuvaj
                        </Text>
                    )}
            ...
            {isProfileLoading ? (
                ...
                <View style={styles.content}>
                    <Text style={styles.label}>Kako se zove tvoj posao?</Text>
                    <TextInput
                        style={[styles.textInput, isFocused && styles.textInputFocused]}
                        placeholder="Npr. Kreator sadržaja, Inženjer"
