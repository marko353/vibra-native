...
const genderOptions = [
  {
    value: "male",
    label: "Muški",
    description: "Identifikujem se kao muškarac.",
    icon: "male",
  },
  {
    value: "female",
    label: "Ženski",
    description: "Identifikujem se kao žena.",
    icon: "female",
  },
  {
    value: "other",
    label: "Prikazi oba pola",
    description: "Prikazuje oba pola.",
    icon: "user-secret",
  },
];
...
export default function GenderScreen() {
  ...
  const updateProfileMutation = useMutation({
    ...
    onError: (error: any) => {
      console.error(
        "Greška pri čuvanju pola:",
        error.response?.data || error.message
      );
      Alert.alert(
        "Greška",
        `Došlo je do greške prilikom čuvanja pola: ${error.response?.data?.message || error.message}`
      );
    },
  });
  ...
  const handleSave = () => {
    ...
    if (!hasChanges || updateProfileMutation.isPending || isProfileLoading)
      return;
  ...
  const renderItem = ({
    ...
    item: { value: string; label: string; description: string; icon: any };
  }) => {
    const isSelected = item.value === selectedGender;
    return (
      <TouchableOpacity
        style={[
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    paddingTop:
      Platform.OS === "android"
        ? (StatusBar.currentHeight ?? 0) + wp(2)
        : wp(2.5),
    borderBottomWidth: 1,
