...
const OPTIONS = [
  { value: 'Long-term partner', emoji: '💍', description: 'Looking for a serious, lasting relationship.' },
  { value: 'New friends', emoji: '🤝', description: 'Here to meet new people and build friendships.' },
  { value: 'Something casual', emoji: '✨', description: 'Open to fun, no-pressure connections.' },
  { value: 'Still figuring it out', emoji: '🤔', description: 'Not sure yet, keeping an open mind.' },
];
...
export default function RelationshipTypeScreen() {
  ...
  useEffect(() => {
    if (userProfile?.relationshipType) setSelected(userProfile.relationshipType);
  }, [userProfile]);

  const hasChanges = useMemo(() =>
    selected !== userProfile?.relationshipType,
  ...
  const mutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      setProfileField('relationshipType', variables.value);
      queryClient.setQueryData(['userProfile', user?.id], (old: any) =>
        old ? { ...old, relationshipType: variables.value } : old
      );
      router.back();
    },
    onError: (error: any) => {
  ...
  const handleSave = () => {
    ...
    mutation.mutate({ field: 'relationshipType', value: selected });
  };
