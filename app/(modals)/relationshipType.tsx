...
interface UserProfile { relationshipType: string | null; [key: string]: any; }
...
export default function RelationshipTypeScreen() {
    ...
    useEffect(() => {
        if (userProfile?.relationshipType) {
            setSelectedType(userProfile.relationshipType);
        }
    }, [userProfile]);

    const hasChanges = useMemo(() => selectedType !== userProfile?.relationshipType, [selectedType, userProfile]);
    ...
    const updateProfileMutation = useMutation({
        ...
        onSuccess: (data, variables) => {
            const fieldName = 'relationshipType' as const;
        ...
        onError: (error: any) => {
            console.error('Greška pri čuvanju tipa veze:', error.response?.data || error.message);
            Alert.alert('Greška', `Došlo je do greške prilikom čuvanja tipa veze: ${error.response?.data?.message || error.message}`);
        },
    });

    const handleSave = () => {
        if (!selectedType) {
            Alert.alert('Greška', 'Molimo odaberite opciju za tip veze.');
            return;
        }
        if (!hasChanges || updateProfileMutation.isPending || isProfileLoading) return;
        updateProfileMutation.mutate({ field: 'relationshipType', value: selectedType });
    };
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle}>Tip veze</Text>
                <TouchableOpacity
                    style={styles.saveBtn}
                    onPress={handleSave}
                    disabled={!hasChanges || updateProfileMutation.isPending || isProfileLoading || !selectedType}
                >
                    {updateProfileMutation.isPending ? (
                        ...
                        <Text style={[
                            styles.saveBtnText,
                            { color: (!hasChanges || !selectedType) ? COLORS.textSecondary : COLORS.primary }
                        ]}>
                            Sačuvaj
                        </Text>
                    )}
            ...
            {isProfileLoading ? (
                ...
                <View style={styles.content}>
                    <Text style={styles.label}>Odaberite tip veze</Text>
