...
const OPTIONS = [
  { value: 'Yes', emoji: '🚬', description: 'I smoke regularly.' },
  { value: 'No', emoji: '🚭', description: "I don't smoke." },
  { value: 'Occasionally', emoji: '💨', description: 'I smoke from time to time.' },
  { value: 'Trying to quit', emoji: '💪', description: "I'm working on quitting." },
];
...
export default function SmokesScreen() {
  ...
  useEffect(() => {
    if (userProfile?.smokes) setSelected(userProfile.smokes);
  }, [userProfile]);

  const hasChanges = useMemo(() =>
    selected !== userProfile?.smokes,
  ...
  const mutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      setProfileField('smokes', variables.value);
      queryClient.setQueryData(['userProfile', user?.id], (old: any) =>
        old ? { ...old, smokes: variables.value } : old
      );
      router.back();
    },
    onError: (error: any) => {
  ...
  const handleSave = () => {
    ...
    mutation.mutate({ field: 'smokes', value: selected });
  };
