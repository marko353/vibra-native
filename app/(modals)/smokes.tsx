...
interface UserProfile { smokes: string | null; [key: string]: any; }
...
export default function SmokesScreen() {
    ...
    const [selectedSmokes, setSelectedSmokes] = useState<string | null>(null);
    ...
    useEffect(() => {
        if (userProfile?.smokes) {
            setSelectedSmokes(userProfile.smokes);
        }
    }, [userProfile]);

    const hasChanges = useMemo(() => selectedSmokes !== userProfile?.smokes, [selectedSmokes, userProfile]);
    ...
    const updateProfileMutation = useMutation({
        ...
        onSuccess: (data, variables) => {
            const fieldName = 'smokes' as const;
            ...
            setSelectedSmokes(newValue);

            // 4. Zatvaranje modala
            router.back();
        },
        onError: (error: any) => {
            console.error('Greška pri čuvanju statusa pušenja:', error.response?.data || error.message);
            Alert.alert('Greška', `Došlo je do greške prilikom čuvanja statusa pušenja: ${error.response?.data?.message || error.message}`);
        },
    });

    const handleSave = () => {
        if (!selectedSmokes) {
            Alert.alert('Greška', 'Molimo odaberite opciju za pušenje.');
            return;
        }
        if (!hasChanges || updateProfileMutation.isPending || isProfileLoading) return;
        updateProfileMutation.mutate({ field: 'smokes', value: selectedSmokes });
    };

    const renderOption = (option: string) => {
        const isSelected = option === selectedSmokes;
        ...
        return (
            <TouchableOpacity
                ...
                onPress={() => setSelectedSmokes(option)}
                disabled={isDisabled}
            >
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle}>Pušenje</Text>
                <TouchableOpacity
                    style={styles.saveBtn}
                    onPress={handleSave}
                    disabled={!hasChanges || updateProfileMutation.isPending || isProfileLoading || !selectedSmokes}
                >
                    {updateProfileMutation.isPending ? (
                        ...
                        <Text style={[
                            styles.saveBtnText,
                            { color: (!hasChanges || !selectedSmokes) ? COLORS.textSecondary : COLORS.primary }
                        ]}>
                            Sačuvaj
                        </Text>
                    )}
            ...
            {isProfileLoading ? (
                ...
                <View style={styles.content}>
                    <Text style={styles.label}>Da li pušiš?</Text>
